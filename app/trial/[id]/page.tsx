...
import { 
  Search, MapPin, Sparkles, ArrowLeft, Activity, ChevronRight, 
  HelpCircle, SearchX 
} from 'lucide-react';

// --- IMPORT BOTH CARD TYPES ---
import TrialCard from '@/components/TrialCard';         // Original "Netflix" Vertical Card
import TrialCardWide from '@/components/TrialCardWide'; // New "Ticket" Wide Card
...
export default function Home() {
  ...
  useEffect(() => {
    async function getStatsAndFeatured() {
      const { count } = await supabase.from('trials').select('*', { count: 'exact', head: true }).ilike('status', 'recruiting');
      setRecruitingCount(count || 0);

      const { data: featuredData } = await supabase.from('trials').select('*').ilike('status', 'recruiting').order('last_updated', { ascending: false }).limit(6);
    ...
    try {
      ...
      if (searchZip.trim()) {
        ...
        if (geoRes.ok) {
          ...
          setUserState(geoData.places[0]['state abbreviation']); // Capture state for smart location
        } else {
      ...
      if (lat && lon && !isNationwideSearch) {
        const { data, error } = await supabase.rpc('get_trials_nearby', {
          lat, long: lon, miles: distance, search_term: searchQuery.trim() || null
        });
      ...
      if (primaryResults.length === 0) {
        if (searchQuery.trim().length > 2) {
          const { data: suggestionData } = await supabase.rpc('get_spelling_suggestion', { search_term: searchQuery });
          if (suggestionData && suggestionData.length > 0) {
            const bestMatch = suggestionData[0].suggestion;
            if (bestMatch.toLowerCase() !== searchQuery.toLowerCase()) setSpellingSuggestion(bestMatch);
          }
        }

        let rawRecs: any[] = [];
        if (lat && lon) {
          const { data: nearbyData } = await supabase.rpc('get_trials_nearby', { lat, long: lon, miles: distance, search_term: null });
          rawRecs = nearbyData || [];
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (zipCode.trim() && zipCode.trim().length < 5) { alert("Please enter a valid 5-digit Zip Code."); return; }
  ...
  const applySuggestion = () => {
    if (spellingSuggestion) {
      ...
      performSearch(spellingSuggestion, zipCode);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {!hasSearched ? (
        // =========================================================
        // VIEW A: HOME PAGE (CAROUSEL RESTORED TO ORIGINAL)
        // =========================================================
        <>
          <div className="relative bg-white border-b border-slate-100 overflow-hidden">
            ...
            <div className="max-w-5xl mx-auto px-6 pt-24 pb-32 text-center relative z-10">
              ...
              <form onSubmit={handleFormSubmit} className="max-w-4xl mx-auto bg-white p-2 rounded-3xl md:rounded-full border border-slate-200 shadow-2xl shadow-indigo-100/50 flex flex-col md:flex-row items-center gap-2 animate-in fade-in slide-in-from-bottom-10 duration-700 delay-300">
                <div className="relative w-full md:flex-[2] px-4 group">
                  <div className="flex items-center gap-3">
                    <div className="bg-slate-100 p-2.5 rounded-full text-slate-400 group-focus-within:text-indigo-600 group-focus-within:bg-indigo-50 transition-colors"><Search className="h-5 w-5" /></div>
                ...
                <div className="relative w-full md:flex-1 px-4 group border-t md:border-t-0 border-slate-100 pt-3 md:pt-0">
                  <div className="flex items-center gap-3">
                    <div className="bg-slate-100 p-2.5 rounded-full text-slate-400 group-focus-within:text-indigo-600 group-focus-within:bg-indigo-50 transition-colors"><MapPin className="h-5 w-5" /></div>
                    <div className="flex-1">
          ...
          {featuredTrials.length > 0 && (
            <div className="py-16 border-b border-slate-200 bg-slate-50">
              <div className="max-w-7xl mx-auto px-6">
                ...
                {/* CAROUSEL RESTORED TO USE 'TrialCard' (VERTICAL) */}
                <div className="flex overflow-x-auto gap-6 pb-8 snap-x scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent -mx-6 px-6 md:mx-0 md:px-0">
                  {featuredTrials.map((trial) => (
                    <div key={trial.id} className="min-w-[340px] max-w-[340px] snap-center">
          ...
          <div className="max-w-7xl mx-auto px-6 py-20">
            <div className="flex justify-between items-end mb-10">
              <div><h2 className="text-2xl font-bold text-slate-900">Explore by Condition</h2><p className="text-slate-500 mt-2 text-lg">Common research areas recruiting near you.</p></div>
              <Link href="/conditions" className="hidden md:flex items-center gap-1 text-sm font-bold text-indigo-600 hover:text-indigo-800 transition-colors bg-indigo-50 px-4 py-2 rounded-full">View Directory <ChevronRight className="h-4 w-4" /></Link>
            </div>
        ...
        // =========================================================
        // VIEW B: SEARCH RESULTS (USING WIDE CARD)
        // =========================================================
        <div className="max-w-6xl mx-auto px-6 py-10">
          ...
          <div className="mb-10 border-b border-slate-200 pb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            {results.length > 0 && (
                <div className="flex items-center gap-2 text-sm font-bold text-indigo-600 bg-indigo-50 px-4 py-2 rounded-lg">
                    <Sparkles className="h-4 w-4" /> {results.length} Matches Found
                </div>
            )}
          </div>
          ...
          {loading ? (
             <div className="space-y-6">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-48 w-full bg-white rounded-2xl border border-slate-100 shadow-sm animate-pulse"></div>
                ))}
             </div>
          ) : (
            <div className="space-y-16">
              ...
              {results.length === 0 && spellingSuggestion && (
                <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="text-amber-800">
                    ...
                    <button onClick={applySuggestion} className="font-bold underline hover:text-amber-900 focus:outline-none">{spellingSuggestion}</button>
                    <span>?</span>
                  </div>
                </div>
              )}

              {/* PRIMARY RESULTS - USING WIDE CARD */}
              {results.length > 0 ? (
                <div className="flex flex-col gap-6">
                  {results.map((trial) => {
                    const displayTrial = { ...trial, userState: userState || undefined };
                ...
                <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 border-dashed">
                  <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400"><SearchX className="h-8 w-8" /></div>
              ...
              {/* RECOMMENDATIONS (Keep Vertical for Distinction, or Wide if preferred) */}
              {recommendations.length > 0 && (
                <div className="pt-8 border-t border-slate-200">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg"><Sparkles className="h-5 w-5" /></div>
                    <div>
                  ...
                  {/* Grid of Vertical Cards for Recommendations */}
