...
export default function TrialCard({ trial, featured = false }: TrialCardProps) {
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    const shareData = {
      ...
      text: `I found a paid clinical trial for ${trial.condition}.\n\nView details here:`,
      url: shareUrl,
    };
    if (navigator.share) {
  ...
  const formatLocation = (loc: any) => {
    ...
    if (typeof loc === 'string') return loc.split(',').slice(0, 2).join(', ');
    if (Array.isArray(loc) && loc.length > 0) return `${loc[0].city}, ${loc[0].state}`;
    return "Multiple Locations";
  };

  return (
    <div className={`
      relative flex flex-col h-full rounded-2xl transition-all duration-300 group min-h-[380px]
      ${isPremium 
    ...
    `}>
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-4 relative z-20">
          ...
          <button onClick={handleShare} className="relative z-20 p-2 -mr-2 -mt-2 rounded-full text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
            <Share2 className="h-4 w-4" />
          </button>
        </div>

        <h3 className={`text-lg font-bold mb-3 flex-grow transition-colors line-clamp-3 ${isPremium ? 'text-indigo-900 group-hover:text-indigo-600' : 'text-slate-900 group-hover:text-indigo-600'}`}>
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 mb-4 font-medium">
          <span className="flex items-center">
            ...
            {formatLocation(trial.location || trial.locations)}
          </span>
        </div>

        {isPremium && trial.sponsor && (
          <div className="mb-4 flex items-center gap-2">
            <div className="p-1 bg-indigo-50 rounded text-indigo-600"><Building2 className="h-3 w-3" /></div>
            <span className="text-xs font-bold text-slate-600 truncate max-w-[200px]">{trial.sponsor}</span>
          </div>
        )}
      </div>

      <div className={`mt-auto p-4 border-t rounded-b-2xl relative z-20 ${isPremium ? 'bg-indigo-50/50 border-indigo-100' : 'bg-white border-slate-50'}`}>
        <div className={`flex items-center justify-center w-full py-2.5 rounded-xl font-bold text-sm transition-all duration-200 ${isPremium ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-md' : 'bg-slate-50 text-slate-700 hover:bg-indigo-600 hover:text-white'}`}>
